...
#session_i = odor_i
def load_data_sujet(sujet, band_prep, cond, odor_i, session_i):

    path_source = os.getcwd()
    
    os.chdir(os.path.join(path_prep, sujet, 'sections'))

    if cond in ['FR_CV_1', 'FR_CV_2', 'allcond']:

        raw = mne.io.read_raw_fif(f'{sujet}_{odor_i}_{cond}_{band_prep}.fif', preload=True, verbose='critical')

    else:

        raw = mne.io.read_raw_fif(f'{sujet}_{odor_i}_{cond}_{session_i+1}_{band_prep}.fif', preload=True, verbose='critical')
