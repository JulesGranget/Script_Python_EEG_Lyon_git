    ...
    for odor_i in odor_list:
        ...
        for session_i in range(n_session):
            ...
            for n_chan in range(data_tmp.shape[0]):
                ...
                surrogates_coh[session_i, n_chan,:] = compute_surrogates_coh_results[n_chan]

        #### reduce data
        surrogates_coh_mean_session = surrogates_coh.mean(axis=0)

        #### save
        np.save(f'{sujet}_{cond}_{odor_i}_Coh.npy', surrogates_coh_mean_session)
...
def precompute_surrogates_cyclefreq(sujet, band_prep, cond):
    ...
    for odor_i in odor_list:
        ...
        surrogates_cyclefreq = np.zeros((n_session, 3, len(chan_list_eeg), stretch_point_surrogates))
        ...
        for session_i in range(n_session):
            ...
            for n_chan in range(data_tmp.shape[0]):
                ...
                surrogates_cyclefreq[session_i, 0, n_chan, :] = compute_surrogates_cyclefreq_results[n_chan][0]
                surrogates_cyclefreq[session_i, 1, n_chan, :] = compute_surrogates_cyclefreq_results[n_chan][1]
                surrogates_cyclefreq[session_i, 2, n_chan, :] = compute_surrogates_cyclefreq_results[n_chan][2]

        #### reduce data
        surrogates_cyclefreq_mean_allsession = surrogates_cyclefreq.mean(axis=0)
            
        #### save
        np.save(f'{sujet}_{cond}_{odor_i}_cyclefreq_{band_prep}.npy', surrogates_cyclefreq_mean_allsession)
...
def precompute_MVL(sujet, band_prep, cond):
    ...
    for odor_i in odor_list:
        ...
        MVL_surrogates = np.zeros(( n_session, len(chan_list_eeg), n_surrogates_cyclefreq ))
        MVL_val = np.zeros(( n_session, len(chan_list_eeg) ))
        ...
        for session_i in range(n_session):
            ...
            for n_chan in range(data_tmp.shape[0]):
                ...
                MVL_surrogates[session_i, n_chan,:] = compute_surrogates_MVL[n_chan][1]
                MVL_val[session_i, n_chan] = compute_surrogates_MVL[n_chan][0]

            #### verif
            if debug:
                n_chan = 0
                count, values, fig = plt.hist(MVL_surrogates[session_i, n_chan,:])
                plt.vlines(np.percentile(MVL_surrogates[session_i, n_chan,:], 99), ymin=0, ymax=count.max())
                plt.vlines(np.percentile(MVL_surrogates[session_i, n_chan,:], 95), ymin=0, ymax=count.max())
                plt.vlines(MVL_val[session_i, n_chan], ymin=0, ymax=count.max(), color='r')
                plt.show()

        #### reduce data
        MVL_surrogates_mean_session = MVL_surrogates.mean(axis=0)
            
        #### save
        np.save(f'{sujet}_{cond}_{odor_i}_MVL_{band_prep}.npy', MVL_surrogates_mean_session)
