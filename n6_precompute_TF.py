    ...
    for band_prep_i, band_prep in enumerate(band_prep_list):
        ...
        for odor_i in odor_list:
            ...
            for band, freq in freq_band.items():
                ...
                tf_mean_allsession = np.zeros((n_session, len(chan_list_eeg), nfrex, stretch_point_TF))

                #session_i = 0
                for session_i in range(n_session):

                    #### select data without aux chan
                    data = load_data_sujet(sujet, band_prep, cond, odor_i, session_i)

                    #### remove aux chan
                    data = data[:len(chan_list_eeg),:]
                    
                    print(f"{band} : {freq} : {session_i}")
                    print('COMPUTE')
                    ...
                    #### load
                    tf_mean_allsession[session_i, :, :, :] = tf_allband_stretched
                ...
                #### mean session
                tf_mean_allsession = tf_mean_allsession.mean(axis=0)

                #### save
                print('SAVE')
                os.chdir(os.path.join(path_precompute, sujet, 'TF'))

                np.save(f'{sujet}_tf_{str(freq[0])}_{str(freq[1])}_{cond}_{odor_i}.npy', tf_mean_allsession)
